...
namespace sq
{
	...
	struct sq_contract
	{
		...
		price_t limit_up;
		price_t tick;
		product_type_t trade_type;  //
		double unit;
		
		//=====期权用========
		contract_t base_contract; // 标的合约
		contract_t contract_symbol;
		callpull_type_t call_put;

		trade_date_t expire_date; // 到期日
		price_t strike_price;	  // 执行价
		int vol_multipler;		  // 合约单位
