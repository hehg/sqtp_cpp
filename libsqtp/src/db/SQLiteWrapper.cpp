...
SQLiteStatement::SQLiteStatement(std::string const& statement,sqlite3* db) {
	if(sqlite3_prepare(
		...
		-1, /*╢┴╚б╡─╫╓╜┌│д╢╚гм╚Ї╨б╙┌0гм╘Є╓╒╓╣г╗╚Ї┤є╙┌0гм╘Є╬к─▄╢┴╚б╡─╫ю┤є╫╓╜┌╩¤*/
		&stmt_, /*╙├└┤╓╕╧Є╩ф╚ы▓╬╩¤╓╨╧┬╥╗╕Ў╨ш╥к▒р╥ы╡─SQL╙я╛ф┤ц╖┼╡─SQLite statement╢╘╧є╡─╓╕╒ы*/
		0 /*╓╕╒ыг║╓╕╧Єstmt╬┤▒р╥ы╡─▓┐╖╓*/
		)
		!=SQLITE_OK){
...
SQLiteStatement::~SQLiteStatement(){
	...
	// ╙я╖и: int sqlite3_finalize(sqlite3_stmt *pStmt);
...
bool SQLiteStatement::Bind(int pos_zero_indexed,std::string const& value){
	if (sqlite3_bind_text(
		stmt_,
		pos_zero_indexed+1, // ═и┼ф╖√╦ў╥¤
		value.c_str(),
		value.length(), // ╬─▒╛│д╢╚
		SQLITE_TRANSIENT // SQLITE_TRANSIENT: SQLite ╫╘╬╥╕┤╓╞
		)
		!=SQLITE_OK) {
			return false;
	}
