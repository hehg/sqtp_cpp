...
namespace sq{
tcp_handler::tcp_handler():event_handler()
	{
		//create();
		//m_reactor->post_msg(EVENT_MSG_ADD_HANDLER, this);
	}
	tcp_handler::tcp_handler(event_reactor*r):event_handler(r)
	{
		//create();
		//m_reactor->post_msg(EVENT_MSG_ADD_HANDLER, this);
	}
	tcp_handler::tcp_handler(event_reactor*r, fd_t fd) 
	...
	bool tcp_handler::send_data(const char*buf,int len)
	{
	...
	void tcp_handler::get_peer_addr(string &peer_addr)
	{
		if (m_fd != invalid_fd)
		{
