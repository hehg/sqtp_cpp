...
Copyright (C) 2004 Renиж Nyffenegger
...
public:
	...
	public:
		...
		double as_double(int idx) { return atof(fields_[idx].c_str()); }
		int64_t as_int64(int idx) { return atoll(fields_[idx].c_str()); }
		int as_int(int idx) { return atoi(fields_[idx].c_str()); }
		std::string as_string(int idx) { return fields_[idx]; }
	};
